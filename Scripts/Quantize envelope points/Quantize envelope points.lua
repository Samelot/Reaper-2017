...
local Slider = require "slider class"
  ...
  if env == nil then
    ...
    if array_created == true then
      ...
      for i=1, sel_points do
        ...
        value = e_v[i]
        pos = e_pos[i]
        new_pos = pos - compress.val * (pos - reaper.SnapToGrid(0, pos));
    ...
    if add_undo_point == false then 
      add_undo_point = true 
    end -- a flag for "reaper.Undo_OnStateChange"
...
function init()
  gfx.init("Quantize selected envelope points", 300, 100)
  ...
  compress = Slider(10, gui.help_text_y + gfx.texth+10, 250, 15, 0.0, 0.0, 0, 1, "Quantize ->","")
