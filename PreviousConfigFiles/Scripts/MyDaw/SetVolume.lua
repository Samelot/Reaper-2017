...
if not set_vol then
    return
  elseif set_vol == "-inf" then
    set_vol = ""
  elseif not tonumber(set_vol) then
    return
  else
    set_vol = tonumber(set_vol)
    if set_vol > 6 then 
      set_vol = 6
    end
    set_vol = 10^(set_vol / 20)
  end
  
  for i = 1, tr_count do
    local tr = reaper.GetSelectedTrack(0, i-1)
    if tr ~= nil then
      reaper.SetMediaTrackInfo_Value(tr, "D_VOL", set_vol)
 
    end
  end
  reaper.Undo_OnStateChangeEx("Volume", -1, -1)
  
end


local function get_track_y_range(cur_tr)
local function get_track_zoom_offset(tr,y_end,h,scroll) -- WHEN ZOOMIN TRACK AT CERTAIN SCENARIONS (ONLY TWO TRACK ONE EXAMPLE) SCROLL DOES NOT UPDATE,ZOOM DOES NOT UPDATE SO SOME WEIRD SHIT IS GOING ON HERE TO DETERMINE THAT
   ...
   local retval, found = reaper.JS_Window_ListAllChild(main_wnd);
    ...
    local _, _, ztop, _, _ = reaper.JS_Window_GetRect(fl)
    if y_end > (ztop + h - 1) then offset = y_end - ztop - h end
    return offset
end
  local track_window = reaper.JS_Window_Find("trackview", true) -- GET TRACK VIEW
  _, scroll, _, _, _ = reaper.JS_Window_GetScrollInfo(track_window, "SB_VERT") -- GET VERTICAL SCROLL
  _, x_view_start, y_view_start, _, y_view_end = reaper.JS_Window_GetRect(track_window) -- GET ARRANGE VIEW X-Y
  ...
  for tr = idx, trcount do
   ...
   if masvis == 1 then
    y_start = totalh+5  -- TRACK Y START
    else
    y_start = totalh
   end
...
function get_track_zoom_offset(tr)
...
GUI.Main_Update_State = function()
 ...
 GUI.char = gfx.getchar()
  ...
  if  GUI.char == 13 then
   ...
   elseif (GUI.char == 27 and not (  GUI.mouse.cap & 4 == 4
                                or   GUI.mouse.cap & 8 == 8
                                or   GUI.mouse.cap & 16 == 16
                                or  GUI.escape_bypass))
            or GUI.char == -1
            or GUI.quit == true
            or reaper.GetExtState( "MyDaw", "focus_on_volume")==nil
            or reaper.GetExtState( "MyDaw", "focus_on_volume")=="" 
...
tr=getvoltrack()
    ...
    mdvol = reaper.JS_Window_Find( "MDVol", true )
    if tr  then 
    
    track_height =  reaper.GetMediaTrackInfo_Value( tr, "I_WNDH" )
    
    
    
   top, bottom = get_track_y_range(tr)
   local track_window = reaper.JS_Window_Find("trackview", true)
   local twindow = get_track_zoom_offset(tr)
   
   
   
   
    retval, left, mtop, right, mbottom = reaper.JS_Window_GetClientRect(twindow)
    track_width = right - left
    folder_indent = 8
    xcorr=-7+(reaper.GetTrackDepth(vtr)*folder_indent)
    ycorr=-32
     
     pos_x = 0
     pos_y = 0
    
    if track_width >=321 and track_height < 44 then
    pos_x = left + 166 + xcorr
    pos_y = top + 4 + ycorr
    elseif track_width < 321 and track_height < 44 then
    pos_x = left + 105 + xcorr
    pos_y = top +4 + ycorr
    elseif track_width <= 321 and track_height >= 44 then
    pos_x = left + 44 + xcorr
    pos_y = top +25 + ycorr
...
previousdig = reaper.GetExtState( "MyDaw", "previousdig")



if (previousdig and previousdig ~= '') then

self:insertchar(previousdig)
...
function GUI.Textbox:previnsert()
...
GUI.Textbox.keys = {
  ...
  [22] = function(self)
        ...
        return self:ctrlchar(self.previnsert)
...
GUI.name = "MDVol"
GUI.x, GUI.y, GUI.w, GUI.h = 0, 0, 57, 17
...
local value_from_1 = GUI.Val("V")

GUI.Val("V", value_from_1)
