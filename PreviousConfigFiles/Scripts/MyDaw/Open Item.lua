...
path,file,extension = SplitFilename(in_path)



if in_path and in_path ~="" and extension ~="rpp" then
reaper.Main_OnCommand(40528 , 0)---- Item: Select item under mouse cursor
reaper.Main_OnCommand(40009 , 0) ---Item properties: Show media item/take properties
...
function run()
  midieditor = reaper.MIDIEditor_GetActive()
  if not midieditor then reaper.defer(run) else  
SetMidiGrid(take) 
end
end
...
--reaper.MIDIEditor_OnCommand(midieditor, 40468) ----zoom one loop
else
reaper.Main_OnCommand(41816 , 0)----- Sub
