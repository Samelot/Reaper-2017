...
function getsomerms()
 ...
 if itemproc  then
       guiditemString = 0
       guiditemString = reaper.BR_GetMediaItemGUID(itemproc)
...
function Button:draw()
    ...
    -- Get mouse state ---------
          ...
          -- in elm L_down -----
          if self:mouseDown() then a=a+0.7 end
...
function CheckBox:draw()
    ...
    -- Get mouse state ---------
          ...
          -- in elm L_down -----
          if self:mouseDown() then a=a+0.6 end
          -- in elm L_up(released and was previously pressed) --
...
local Midi_Sampler = CheckBox:new(740,410,68,18, 0.1,0.5,0.55,0.6, "","Arial",15,  1,
                              {"Sampler","Trigger"} )
...
-------------------------
local VeloMode = CheckBox:new(820,410,90,18, 0.1,0.5,0.55,0.6, "","Arial",15,  1, -------velodaw
                              {"Use RMS","Use Peak"} )
...
-- Detect Velo time ------------------------------ 
local Gate_DetVelo = H_Slider:new(820,430,90,18, 0.1,0.5,0.55,0.6, "Look","Arial",15, 0.25 )------velodaw
...
-- Detect Velo time ------------------------------ 
local Offset_Sld = H_Slider:new(530,430,208,18, 0.1,0.5,0.55,0.6, "Offset","Arial",15, 0.5 )------velodaw
...
--- Velo Slider --------------------------------------------------------------------
-------------------------------------------------------------------------------------
local Gate_VeloScale = Rng_Slider:new(820,450,90,18, 0.1,0.5,0.55,0.6, "Range","Arial",15, 0.231, 0.79 )---velodaw
...
-------------------------
local OutNote  = CheckBox:new(740,430,68,18, 0.1,0.5,0.55,0.6, "","Arial",15,  1,
...
local Manipulate = CheckBox:new(426,380,50,18, 0.1,0.5,0.55,0.6, "","Arial",13,  1,
                              {"ItemLen","ItemPos", "Fades"} )
...
if (Manipulate.norm_val == 1) or (Manipulate.norm_val == 2) then     
   ...
   if reaper.GetSelectedMediaItem( 0, 0) then reaper.Main_OnCommand(40652, 0)
...
if (Manipulate.norm_val == 1) then   
   ...
   if reaper.GetSelectedMediaItem( 0, 0) then reaper.Main_OnCommand(40797, 0) end -------Icrease rate
...
if (Manipulate.norm_val == 1) then   
   ...
   if reaper.GetSelectedMediaItem( 0, 0) then reaper.Main_OnCommand(40798, 0) end -------Decrease rate
...
PPicth.onClick = 
function()
   if reaper.GetSelectedMediaItem( 0, 0) then reaper.Main_OnCommand(40566, 0)
   reaper.Main_OnCommand(40485, 0) ------Set Timebase to position only
...
Slice_RX.onClick = 
function()
   if Wave.State then Wave:Slice_RX() end 
end 
...
--- Button_TB --------------------------
----------------------------------------
local Button_TB = {Get_Sel_Button,Slice_RX,Just_Slice,Add_Markers,Create_MIDI, Midi_Sampler,Manipulate, TransposeUp,TransposeDown, Shorten, Lengthen, Rrate, PPicth, TBase}
...
-------------------------
local NoteLenghth  = CheckBox:new(750,430,90,18, 0.3,0.5,0.5,0.3, "","Arial",15,  5,
...
local Guides  = CheckBox:new(530,410,208,18, 0.1,0.5,0.55,0.6, "","Arial",15,  1,
...
--------------------------------------------------
-- View Checkboxes -------------------------------
local DrawMode = CheckBox:new(965,380,55,18, 0.1,0.5,0.55,0.6, "Draw: ","Arial",15,  3,
                              { "Slowest", "Slow", "Medium", "Fast" } )

-- DrawMode.onClick = Get_Sel_Button.onClick (Отключено(работает только при захвате))
-------------------------
local ViewMode = CheckBox:new(965,400,55,18,  0.1,0.5,0.55,0.6, "Show: ","Arial",15,  1,
...
if  Guides.norm_val == 2 then  reaper.Main_OnCommand(40780, 0)
elseif Guides.norm_val == 3 then reaper.Main_OnCommand(40779, 0)
elseif Guides.norm_val == 4 then reaper.Main_OnCommand(40778, 0)  
elseif Guides.norm_val == 5 then reaper.Main_OnCommand(40776, 0)
elseif Guides.norm_val == 6 then reaper.Main_OnCommand(40775, 0)
elseif Guides.norm_val == 7 then reaper.Main_OnCommand(40774, 0)
end
  Grid_Points_r ={}
 Grid_Points = {}
 grinline = sel_start_g 
   while (grinline <= sel_end_g) do
    grinline = reaper.BR_GetNextGridDivision(grinline)
...
reaper.PreventUIRefresh(1)
   ...
   sourcetrack =  reaper.GetMediaItem_Track(item)
    ...
    tempguid = reaper.BR_GetMediaTrackGUID(temptrack)
 ...
 if (Guides.norm_val == 1) then      
      ...
      for i= 1, points_cnt, 2 do
        ...
        if i<points_cnt-2 then next_startppqpos = (self.sel_start + Gate_Gl.Res_Points[i+2]/srate )
...
temptrack = reaper.BR_GetMediaTrackByGUID( 0, guidetrack )
...
reaper.Main_OnCommand(40718,0)----Select all items on selected tracks in currient time selection
...
function Wave:Create_Track_Accessor() 
 ...
 local item = reaper.GetSelectedMediaItem(0,0)
    if item then
    item_to_slice = reaper.BR_GetMediaItemGUID(item)
